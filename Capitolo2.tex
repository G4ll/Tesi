...
\begin{lemma}
	Uno $ G $-modulo $ A $ è coomologicamente banale se e solo se è coomologicamente come $ G_p $-modulo per ogni $ p $ primo.
\end{lemma}
\begin{proof}
...
\begin{proof}
	...
	\end{equation}
	Per il lemma \ref{scappa}, $ \Hom_Z(A/A^G, \, F) $ è indotto. \todo[dimostarre il lemma mistico nell'opportuna sezione] Nella la successione esatta lunga associata alla $ \ref{homsucc1} $, poiché $ \HH^1(G, \, \Hom_\Z(A/A^G, \, F)) = 0 $, troviamo dunque la suriezione
