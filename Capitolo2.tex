...
Con un piccolo sforzo riusciamo a rimuovere l'ipotesi di $ p $-torsione, ottenendo un magnifico criterio per la banalità di un modulo. Per quanto osservato a inizio capitolo (\ref{banSylow}), continuiamo a enunciare i teoremi per $ p $-gruppi sottolineando però la facilità con cui si estendono al caso generale.
...
Osserviamo, in modo improvviso e forse addirittura inopportuno, che estendendo i coefficienti di un modulo ne preserviamo la banalità.
...
\begin{proof}
	...
	Per concludere, dimostriamo per décalage che i gruppi di coomologia di gradi maggiori $ q > n+1 $, per ogni modulo $ A $ di $ p $-torsione, sono anch'essi nulli. È sufficiente scrivere il modulo di coomologia traslata $ A_{r} $, osservare che anch'esso è di $ p $-torsione (perché l'indotto $ \Ind_1^G(A) $ è composto da mappe che per essere continue devono essere localmente costanti, dunque con immagine finita), e concludere dunque che	
	\begin{equation*}
	 \Hh^{n+1+r}(G, \, A) = \Hh^{n+1}(G, \, A_r) = 0.
	\end{equation*}
\end{proof}
