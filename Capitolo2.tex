...
\begin{proof}
	...
	\begin{equation*}
		...
		\end{tikzcd}
	\end{equation*}
	nella cui successione esatta lunga, poiché $ \Hom_\Z(A, \, R/pR) $ è indotto per il solito lemma che siamo dimenticati di enunciare (??), troviamo
	\[ \begin{tikzcd}[column sep = small]
	...
	Nel caso $ A $ non sia uno $ \Z $-modulo libero, $ R $ lo è e soddisfa le stessi ipotesi. Per quanto appena mostrato, $ R $ è coomologicamente banale, da cui deduciamo che, prendendo la successione esatta lunga associata a \ref{projban3}, anche $ A $ dev'essere coomologicamente banale.
