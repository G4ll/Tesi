...
Questi due risultati, fondamentalmente, traducono nel linguaggio coomologico le proprietà della struttura di un campo locale. Non c'è bisogno di aggiungere che non c'è speranza di ottenere risultati analoghi per campi globali. Nel prossimo paragrafo sfrutteremo quanto appena scoperto per calcolare la coomologia in grado 2.
...
\begin{theorem}
	Il gruppo $ \Br \K $ coincide in realtà con il sottogruppo $ \Br (\knr/K)$.
\end{theorem}
...
Torniamo ora alla dimostrazione del teorema. Per il classico lemma di passaggio al limite, sappiamo che $ \Br \K = \HH^2(\Gamma_K, \, \bar{\K}^\times) $ è limite induttivo sulle sottoestensioni di Galois finite, che per il secondo lemma possiamo calcolare sulle sole estensioni non ramificate:
...
\begin{proof}
	Assumiamo $ i $. Per ogni $ L $, la $ p $-dimensione coomologica del sottogruppo $ \Gal{\bar{k}/L} $ è al più quella del gruppo $ \Gal{\bar{k}/k} $, ne segue che
	...
	che è nullo per ipotesi; da cui, sfoderando tutti i teoremi sulla dimensione coomologica a nostra conoscenza, $ \cd_p(k) \leq \cd_p(L) = 1 $.
\end{proof}
