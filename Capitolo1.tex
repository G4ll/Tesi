...
%% Cotruzione esplicita e cocatene
...
\begin{proposition}\label{funct}
	Siano  $ A $ uno $ G $ modulo e $ A' $ un $ H $ modulo. Dati $ f \colon H \to G $ un omomorfismo di gruppi e $ u \colon \f A \to A' $ un omorfismo $ H $-equivariante, otteniamo una mappa in coomologia
	\[ \HH^i(G, \, A) \to \HH^i(H, \, A') \qquad\forall i \geq 0. \]
\end{proposition}

\begin{proof}
	Presa una risoluzione proiettiva di $ \Z $ nella categoria degli $ G $-moudli
	\[ \dots \to P_2 \to P_1 \to P_0 \to \Z \to 0 \]
	e osserviamo che, poiché il funtore $ \f $ preserva i proiettivi, anche
	\[ \dots \to \f P_2 \to  \f P_1 \to \f P_0 \to \Z \to 0 \]
	è una risoluzione proiettiva, ora di $ H $-moduli. Ci troviamo quindi nella seguente situazione:
	\[\begin{tikzcd}[column sep = small]
	\dots \rar
	& \f P_2 \rar \dar["\id"]
	& \f P_1 \rar \dar["\id"]
	...
	& 0
	\end{tikzcd} \]
	Possiamo ora prendere $ \Hom_H(\,\bullet\, ,\, \f A) $ della prima riga e $ \Hom_G(\,\bullet\, ,\, A) $ della seconda; per funtorialità di $ \f $ otteniamo un morfismo di complessi che in coomologia è
...
\begin{profinite}
Possiamo ora enunciare il risultato grazie al quale siamo riusciti a cofinare questa discussione in queste piccole parentesi: la coomologia dei gruppi profiniti si riconduce a quella dei gruppi finiti.
